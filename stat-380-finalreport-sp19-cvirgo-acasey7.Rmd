...
head(grep("[YEARS]", Heart_age, value = TRUE))
```
...
```{r}
Heart_KData <- HeartData
for(i in 1:length(Heart_KData$`Age Range`)){
  if(Heart_KData$`Age Range`[i] == "44 Years and Below"){
    Heart_KData$`Age Range`[i] <- 0
  }
  else if(Heart_KData$`Age Range`[i] == "45 to 64 Years"){
    Heart_KData$`Age Range`[i] <- 1
  }
  else if(Heart_KData$`Age Range`[i] == "65 Years and Older"){
    Heart_KData$`Age Range`[i] <- 2
  }
}

for(i in 1:length(Heart_KData$Locality)){
  if(Heart_KData$Locality[i] == "Nonmetropolitan"){
    Heart_KData$Locality[i] <- 0
  }
  else if(Heart_KData$Locality[i] == "Metropolitan"){
    Heart_KData$Locality[i] <- 1
  }
}

for(i in 1:length(Heart_KData$State)){
  if(Heart_KData$State[i] == "Pennsylvania"){
    Heart_KData$State[i] <- 0
  }
  else if(Heart_KData$State[i] == "Texas"){
    Heart_KData$State[i] <- 1
  }
  else if(Heart_KData$State[i] == "Washington"){
    Heart_KData$State[i] <- 2
  }
}
```
...
KResults <- kmodes(Heart_KData, 10, iter.max = 10, weighted = FALSE)
KResults

plot(Heart_KData[],col= KResults$cluster)
```
